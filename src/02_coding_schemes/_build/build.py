...
ROOT = os.path.dirname(HERE)                      # .../src/02_coding_schemes
...
def fb_widget(fb_id, label, compact=False, title="Expert feedback"):
    cls = "fb fb-compact" if compact else "fb"
    return (
        f'<div class="{cls}" data-fb-id="{h(fb_id)}" data-fb-label="{h(label)}">'
        f'<div class="fb-head"><span class="fb-title">{h(title)}</span>'
...
def render_subfields(subfields):
    if not subfields:
        return ""
    out = ['<div class="subfields"><div class="sflab">Fields inside each item</div>']
    for sf in subfields:
        vals = ""
        if sf.get("values"):
            vals = '<div class="sfvals">' + "".join(
                f'<span class="sfval">{h(value)}</span>' for value in sf["values"]) + '</div>'
        out.append(
            '<div class="sfrow">'
            f'<span class="sfname">{h(sf["name"])}</span>'
            f'<span class="sfdesc">{h(sf["desc"])}</span>{vals}</div>'
        )
    out.append('</div>')
    return "".join(out)


def render_field(f, section_id=None, with_feedback=False):
    out = ['<div class="field">']
    out.append(f'<span class="fname">{h(f["name"])}</span>')
    chips = []
    if f.get("kind"):
        chips.append(f'<span class="fkind">{h(f["kind"])}</span>')
    if f.get("cap"):
        chips.append(f'<span class="fkind cap">max {h(f["cap"])} items</span>')
    if f["free_text"]:
        chips.append('<span class="fkind free">free text</span>')
    out.append('<div class="fconstruct">' + h(f["construct"]) + (" " + "".join(chips) if chips else "") + '</div>')
    out.append(render_values(f["values"]))
    out.append(render_subfields(f.get("subfields")))
    if f["notes"]:
        out.append(f'<div class="fnotes">{h(f["notes"])}</div>')
    if with_feedback and section_id:
        out.append(fb_widget(f"{section_id}::{f['name']}", f["name"], compact=True,
                             title="Expert feedback on this field"))
    out.append('</div>')
    return "".join(out)
...
def render_section(sec):
    ...
    if kind == "prose":
        ...
        field_feedback = bool(sec.get("field_feedback"))
        if field_feedback:
            parts.append('<p class="fbhint">Two levels of feedback in this section. Every field '
                         'below carries its own <b>Expert feedback on this field</b> box, and the '
                         'box at the end of the section is <b>Expert feedback on this section as a '
                         'whole</b>. Use whichever fits what you want to say.</p>')
        for f in sec["fields"]:
            parts.append(render_field(f, section_id=sec["id"], with_feedback=field_feedback))
    elif kind == "taxonomy":
    ...
    if sec.get("feedback"):
        label = ("Expert feedback on this section as a whole"
                 if sec.get("field_feedback") else "Expert feedback on this section")
        parts.append(fb_widget(sec["id"], title, title=label))
...
def scheme_html(s):
    ...
    has_proposed = any(sec.get("enhancement") for sec in s["sections"])
  ...
  <div class="status"><span class="ic">!</span><div><b>Draft for expert evaluation.</b>
    {h(C.PROJECT["status_long"])} This surface lets you record a verdict and comments and export them as JSON.</div></div>
  ...
  <div class="toolbar">
    ...
    howto = howto.replace(
        "__FILTER_BUTTON__",
        '<button class="btn sm" id="btn-filter" aria-pressed="false" '
        'title="Show only the sections marked Proposed">Show only proposed<span class="kbd">p</span></button>'
        if has_proposed else "",
    )

    overall = f'''<div id="overall" class="overall">
  <h2>Overall assessment</h2>
  <p class="intro">Last step. Identify yourself, give an overall verdict on the scheme as a whole, and export. The per-section and per-field verdicts you recorded above are already included in that export.</p>
  <div class="reviewer">
  ...
  {fb_widget("overall", "Overall assessment of Scheme " + str(num), title="Expert feedback on the scheme as a whole")}
  <div class="endbar">
  ...
  <p class="intro">Where this scheme is implemented and specified in the repository.</p>
