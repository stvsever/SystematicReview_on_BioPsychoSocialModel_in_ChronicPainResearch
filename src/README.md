...
│       │
│       └── fulltext/           # cross-provider test run, full text (scheme 3)
...
Code refers to a section by its plain name (`data`, `config`, `review_stages`, `semantic`, ...)
through `project_path()`. The mapping from that name to the numbered directory sits in one
place, `bps_review/utils/paths.py`, so the physical grouping can change without touching a
single call site.
...
## The two test-run pipelines in one line each
...
fulltext = run_fulltext_testrun_pipeline()            # reuse cached corpus and codings
```
