...
from bps_review.llm.openrouter import chat_completion_json_with_usage, resolve_default_model
...
# The exact output contract, one entry per field of Stage2StructuredRecord.
#
# This block exists because a prompt that only lists value vocabularies does not
# tell a model which keys to return. Without it, models answered with the fields
# whose option lists they could see and silently omitted the rest, which the
# repair layer then filled from the deterministic rule-based coder. The coding
# looked complete and was in fact lexical for those fields. Naming every field,
# with its anchor and its allowed values, is what makes the response a coding
# rather than a partial one.
FIELD_SPECIFICATION: dict[str, dict[str, object]] = {
    "record_id": {
        "instruction": "Echo the record_id of the record you are coding, exactly as given.",
    },
    "review_type": {
        "instruction": "Evidence-synthesis design, as stated in the abstract or the publication types.",
        "values": REVIEW_TYPE_OPTIONS,
    },
    "objective_category": {
        "instruction": "The primary aim of the review. 'conceptual' when the aim is to clarify, define, "
                       "or map a construct or model; 'clinical' when it is about treatment or care; "
                       "'methodological' when it is about how research is done; 'epidemiological' when "
                       "it is about distribution or burden; 'mixed' when several aims genuinely apply.",
        "values": OBJECTIVE_CATEGORY_OPTIONS,
    },
    "icd11_pain_category": {
        "instruction": "The ICD-11 chronic pain family the review actually studies. Use 'mixed or "
                       "unspecified chronic pain' when several families are covered or the abstract "
                       "does not narrow it down.",
        "values": ICD11_OPTIONS,
    },
    "musculoskeletal_flag": {
        "instruction": "Whether this record is relevant to the musculoskeletal chronic pain review. "
                       "'yes' when a musculoskeletal condition is studied (low back, neck, "
                       "osteoarthritis, fibromyalgia, shoulder, and similar); 'no' when the pain "
                       "family is clearly something else; 'unclear' when musculoskeletal relevance "
                       "cannot be ruled out from the abstract.",
        "values": ["yes", "no", "unclear"],
    },
    "bps_function": {
        "instruction": "What the biopsychosocial model is actually doing in this abstract.",
        "values": BPS_FUNCTION_OPTIONS,
    },
    "bio_mentioned": {
        "instruction": "Whether the abstract carries substantive biological content: anatomy, "
                       "physiology, pathophysiology, nociception, sensitization, inflammation, "
                       "imaging, genetics, pharmacology, or tissue pathology. Judge the content, "
                       "never the label.",
        "values": ["yes", "no"],
    },
    "psych_mentioned": {
        "instruction": "Whether the abstract carries substantive psychological content: cognition, "
                       "affect, behaviour, beliefs, coping, catastrophizing, fear-avoidance, "
                       "self-efficacy, depression, anxiety, acceptance, or psychological treatment.",
        "values": ["yes", "no"],
    },
    "social_mentioned": {
        "instruction": "Whether the abstract carries substantive social content: work and occupational "
                       "context, family and relationships, culture, socioeconomic position, healthcare "
                       "system, social support, stigma, or policy. The bare token 'psychosocial' is "
                       "not social content.",
        "values": ["yes", "no"],
    },
    "quality_assessment_reported": {
        "instruction": "Whether the abstract reports a risk-of-bias or quality assessment (AMSTAR, "
                       "GRADE, Cochrane RoB, or a named quality appraisal).",
        "values": ["yes", "no", "unclear"],
    },
    "psychological_concepts_detected": {
        "instruction": "The psychological constructs the abstract names, as a normalized list of short "
                       "lowercase labels. Return an empty list when it names none.",
        "type": "list of strings",
    },
    "theoretical_frameworks_detected": {
        "instruction": "The theoretical models or frameworks the abstract names, as a normalized list "
                       "of short lowercase labels. Return an empty list when it names none.",
        "type": "list of strings",
    },
    "conceptual_problem_flags": {
        "instruction": "Conceptual problems visible in the abstract. Return ['none'] when you see none.",
        "values": CONCEPTUAL_PROBLEM_OPTIONS,
        "type": "list of values from the list above",
    },
    "provisional_typology": {
        "instruction": "What this abstract signals about how the biopsychosocial model is used. This "
                       "is a signal read from an abstract, not a full-text verdict.",
        "values": TYPOLOGY_OPTIONS,
    },
    "stage3_candidate": {
        "instruction": "Whether the record should go forward to full-text coding for the "
                       "musculoskeletal review.",
        "values": ["yes", "no"],
    },
    "stage3_priority": {
        "instruction": "How urgently the record should be read at full text.",
        "values": STAGE3_PRIORITY_OPTIONS,
    },
    "coding_rationale": {
        "instruction": "At most 40 words justifying the typology and the domain judgements.",
    },
}
...
def _batch_prompt(batch: list[dict[str, str]]) -> str:
    instructions = {
        ...
        "output_contract": [
            "Return one JSON object with the single key 'records', holding one item per input record.",
            "Every item must contain every key listed under 'fields'. Never omit a key.",
    ...
    return (
        ...
        "Return one JSON object with key 'records'. Each item must contain every field listed under "
        "'fields' in the instructions, using exactly those key names. "
...
def _parse_batch(
    ...
    response_payload, usage = chat_completion_json_with_usage(
        prompt,
        model=model,
        temperature=0.0,
        system_prompt=(
