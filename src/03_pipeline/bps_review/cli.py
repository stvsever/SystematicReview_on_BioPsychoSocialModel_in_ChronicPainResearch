...
def main() -> None:
    ...
    abstract_testrun = subparsers.add_parser(
        "run-abstract-testrun",
        help="Cross-provider abstract-level test run: code the PubMed sample once per model and analyse agreement",
    )
    abstract_testrun.add_argument("--force-corpus", action="store_true", help="Retrieve a fresh PubMed sample")
    abstract_testrun.add_argument("--force-coding", action="store_true", help="Re-code every abstract via the API")

    fulltext_testrun = subparsers.add_parser(
        "run-fulltext-testrun",
        help="Cross-provider full-text test run: code the retrieved open-access corpus once per model",
    )
    ...
    subparsers.add_parser(
        ...
        help="Retrieve the open-access full texts of the abstract-level candidate set",
    )
    subparsers.add_parser(
        "run-all",
    ...
    if args.command == "search-pubmed":
        ...
        print(json.dumps(summary, indent=2))
    elif args.command == "run-abstract-testrun":
        from bps_review.pilot import run_abstract_testrun

        out = run_abstract_testrun(force_corpus=args.force_corpus, force_coding=args.force_coding)
        print(json.dumps({
            "abstracts": len(out["corpus"]),
        ...
        frame = build_fulltext_corpus()
        print(json.dumps({"fulltexts_retrieved": len(frame)}, indent=2))
