...
{\small\itshape The operational Stage 2 vocabulary. Anchors and boundaries below are the proposed reliability layer over the existing value lists.}\par\smallskip
\begin{description}
...
\item[\texttt{musculoskeletal\_flag}] Whether the review concerns musculoskeletal pain.
  \begin{itemize}[leftmargin=1.3em,itemsep=2pt,topsep=2pt]
...
\item[\texttt{bio\_mentioned / psych\_mentioned / social\_mentioned}] Binary presence of each domain in the abstract.
  ...
  \end{itemize}
{\small See the proposed graded-mention refinement below; the current field is binary.}
...
\item stage3\_candidate is forced to yes whenever musculoskeletal\_flag is yes or unclear.
...
\section*{Proposed Refinement: Graded Domain Mention and Dual Function}
\refbadge
Refinement proposed for expert evaluation. Two changes would raise the resolution of Stage 2 without changing its scope.

First, replace the binary bio\_mentioned, psych\_mentioned, and social\_mentioned with a three-level scale: substantive (a named construct is discussed), lexical (only the domain umbrella word appears), and absent. This mirrors the S\_lex versus S\_subst distinction already used in the manuscript audit and lets the social-shortfall finding be measured at the abstract level rather than only after recoding.

Second, allow bps\_function to carry a dominant function plus an optional secondary function. Many abstracts both organize scope and justify treatment; forcing a single label discards that signal and depresses agreement on mixed abstracts.

Third, reinstate optional spiritual\_existential and lifestyle mention flags. The corpus contains explicit biopsychosocial-spiritual reviews and religious-belief reviews, so the extra flags capture real variance that the current binary domains drop.


\section*{Worked Examples (real corpus records)}
{\small\itshape Actual Stage 2 codings from the test-run manifest, shown as reliability anchors.}\par\smallskip
\begin{description}
\item[Where has the 'bio' in bio-psycho-social gone?] {\small \textit{Narrative or expert review, 2019}}\\
{\small Coding: \texttt{provisional\_typology} = pseudo-bps or partial signal; \texttt{conceptual\_problem\_flags} = parallel\_listing\_without\_integration; \texttt{bps\_function} = organizing principle.\\ \textit{Why:} The title itself signals a missing biological pole, so the typology is a partial signal and the flags record the imbalance.}
\item[The biopsychosocial model is lost in translation: from misrepresentation to an enactive modernization] {\small \textit{Narrative or expert review, 2023}}\\
{\small Coding: \texttt{provisional\_typology} = potential integrative signal; \texttt{conceptual\_problem\_flags} = vague\_definition | tokenistic\_bps; \texttt{bps\_function} = explanatory framework.\\ \textit{Why:} A conceptual, theory-facing review can be an integrative signal and still be flagged for definitional vagueness. The two axes are independent.}
\item[Exercise for chronic musculoskeletal pain: a biopsychosocial approach] {\small \textit{Narrative or expert review, 2017}}\\
{\small Coding: \texttt{icd11\_pain\_category} = chronic secondary musculoskeletal pain; \texttt{stage3\_candidate} = yes; \texttt{provisional\_typology} = potential integrative signal.\\ \textit{Why:} Musculoskeletal focus forces the Stage 3 candidate flag; the integrative signal sets a high retrieval priority.}
\end{description}

\section*{Documented Divergences}
{\small\itshape Where prose, CSV codebook, and outputs disagree, the output schema is treated as canonical.}\par\smallskip
\begin{itemize}
\item The prose codebook lists treatment-oriented as an objective class, but the implemented prompt and output schema do not use that label.
\item The CSV codebook historically contains spiritual\_existential\_mentioned and lifestyle\_mentioned; those fields are absent from the current output file and are therefore not canonical here. The refinement above proposes reinstating them explicitly.
\item Prose and code both emphasize that BPS language alone is insufficient evidence of true triadic coverage.
\item The test-run fallback forces stage3\_candidate to yes only from musculoskeletal\_flag. For the two-review design a parallel neuropathic flag should drive the neuropathic candidate pool; this is a proposed change awaiting expert sign-off.
\end{itemize}
