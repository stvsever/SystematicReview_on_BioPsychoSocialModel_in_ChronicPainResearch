...
The study was designed as a pre-registered, staged mixed-method systematic review and reported using PRISMA-oriented flow logic \citep{page2021, moher2009} adapted for conceptual synthesis. The OSF registration defines the governing framework for eligibility, coding, reliability checks, and reconciliation; the implemented pipeline preserves those constraints by separating automated preprocessing from human decision authority at eligibility and adjudication steps.

\subsection{Computational Reproducibility}
All operational steps were executed through version-controlled command-line stages in the \texttt{bps\_review} package (Python 3.11 environment), with configuration loaded from the project \texttt{config/} files. The run order was database retrieval, deduplication, screening preparation, Stage~1 screening, Stage~2 extraction, Stage~3 preparation, semantic loading, and asset build. Each stage writes structured outputs to fixed \texttt{review\_stages/} and \texttt{paper/assets/tables} locations, enabling exact reruns and file-level audit trails. Deterministic settings were enforced for stochastic procedures (for example, fixed random seed in projection steps).
...
\end{itemize}
Records were excluded if they were primary studies, protocols without results, editorials, commentaries, letters, conference abstracts, books or book chapters, pediatric-only populations, acute-pain-only studies, animal studies, non-English language sources, or grey literature.

\subsection{Information Sources and Search Execution}
Searches were executed in MEDLINE (via PubMed API) and Web of Science Core Collection (Starter API). The protocol also specifies PsycINFO (EBSCOhost/EDS); PsycINFO retrieval was unavailable in this run because credentials were not available at execution time and was logged as a protocol deviation. The operational date window was January~1977 to March~2026. Registered search expressions combine biopsychosocial lexical variants with chronic-pain and review-design filters; all query keys, retrieval metadata, and normalized exports are stored in the audit outputs.

\subsection{Screening, Deduplication, and Decision Governance}
All retrieval outputs were normalized to a common schema before deduplication. Deduplication used title/abstract similarity and identifier checks, then produced a single merged screening set. Stage~1 applied explicit title/abstract decision rules derived from the registered criteria and generated exclusion labels, borderline flags, and reliability materials. In line with the OSF framework, reliability templates target 20\% double-screening subsets (capped at 50 for Stage~1/2 and 20 for Stage~3 full-text checks). Discrepancies are resolved by consensus and principal investigator adjudication. Titles, abstracts, authors, journal, and year remained visible (no masking).

\subsection{Stage 2 Extraction and Coding}
Stage~2 extracted bibliographic metadata and BPS-operationalization variables from title and abstract for all Stage~1-included records. The implemented extraction stack combined deterministic metadata parsing with structured LLM-first coding using schema-constrained outputs, archived response payloads, and post-hoc validation. Variables included review design, objective category, ICD-11 pain category \citep{treede2019, nicholas2019}, BPS mention location/function, substantive domain indicators, quality-reporting indicators, psychological concepts, framework mentions, conceptual-problem flags, provisional typology, and auditable rationale fields. To avoid lexical inflation, coding rules explicitly disallowed counting the token ``biopsychosocial'' as sufficient evidence of triadic substantive coverage.

\subsection{Stage 3 Full-Text Preparation, Retrieval, and Manual Governance}
Stage~3 preparation targeted musculoskeletal and musculoskeletal-unclear records for full-text deep coding (RQ2/RQ3). The pipeline generated a candidate manifest with retrieval source, retrieval status, cached-text path, content-length checks, manual retrieval flags, and manual relevance flags. Open-access retrieval from PubMed Central was attempted first; unresolved records were routed to a manual retrieval queue. A run-level manual relevance checklist and OSF-alignment checklist were exported so that final inclusion relevance and interpretive coding remain human-adjudicated, consistent with the registration's governance model.

\subsection{Semantic Loading Analysis}
To complement structured coding, we applied an ontology-aligned semantic loading procedure over a shared embedding space. For each record, a composite text (title + abstract + objective) was embedded using \texttt{text-embedding-3-small}. Ontology prompts for 42 subdomains (10 biological, 20 psychological, 12 social) were embedded independently; cosine similarities were softmax-normalized to obtain record-level domain loadings and weighted subdomain contributions. Pairwise products (bio$\times$psycho, bio$\times$social, psycho$\times$social) and triadic products were computed to quantify co-loading intensity relative to equal-balance benchmarks. For landscape visualization, joint record-plus-ontology embeddings were projected with PCA-preconditioned t-SNE under fixed random seed settings to preserve rerun stability.

\subsection{Synthesis Strategy and Inference Boundaries}
The synthesis integrates descriptive statistics (frequencies, cross-tabulations, proportions, and benchmark-relative deviations), provisional typological classification, and qualitative interpretation. Stage~2 assigns one of four provisional operationalization categories: potential integrative signal, multifactorial signal, rhetorical label signal, and pseudo-BPS or partial signal. These categories are explicit interim bridges to the Stage~3 full-text typology (True Integrative BPS, Multifactorial, Pseudo-BPS, Rhetorical BPS). Because Stage~3 retrieval and adjudication are ongoing, integration-quality claims are interpreted as provisional and bounded to currently available full-text evidence.

\subsection{Pre-registration}
The review was pre-registered prior to data collection (Van Severen et al., 2026). The registration defines screening, extraction, reliability, reconciliation, and governance expectations; run-specific deviations (for example, source-availability constraints) are logged in reproducible audit outputs and supplementary documentation.
...
\begin{figure}[htbp]
  \caption{PRISMA-style study selection workflow from identification through full-text planning.}
  \label{fig.prisma}
  \centering
  \includegraphics[width=0.98\textwidth]{../assets/figures/prisma_flow.png}
...
\captionof{table}{Characteristics of all 111 included reviews.}
\label{tab.characteristics}
\input{generated/characteristics_table.tex}
...
\input{generated/primary_answers.tex}
...
\input{generated/secondary_answer.tex}
...
\label{app:methods}
\input{generated/supplement_methods.tex}

\section{Supplementary Semantic Tables}
\label{app:semantic}
\input{generated/semantic_domain_summary_table.tex}
\input{generated/semantic_pairwise_summary_table.tex}
